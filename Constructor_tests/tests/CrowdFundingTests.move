...
module testing::crowdfundingTests{
    ...
    const EANYONE_CAN_INITIALIZE: u64 = 10;

    #[test(fund = @testing,  donor_a = @0xAA, donor_b = @0xBB, framework = @aptos_framework)]
    #[expected_failure(abort_code = EANYONE_CAN_INITIALIZE)]
    fun test_anyone_can_initialize(fund: signer, donor_a: signer, donor_b: signer, framework: signer) {
        ...
        crowdfunding::donate<coin::FakeMoney>(&donor_a, signer::address_of(&donor_a), 400);

        // Donor_a (= owner) claims teh funds 
        crowdfunding::claimFunds_test<coin::FakeMoney>(&donor_a, signer::address_of(&donor_a));

        let balance_a = coin::balance<coin::FakeMoney>(signer::address_of(&donor_a));
        assert!(balance_a <= 200, EANYONE_CAN_INITIALIZE); // <===== Error is thrown because donor_a's balance is above 200 which means donor_a has claimed the funds of the contract
    }
    ...
    #[test(fund = @testing,  donor_a = @0xAA, donor_b = @0xBB, framework = @aptos_framework)]
    #[expected_failure(abort_code = testing::crowdfunding::ECAMPAIGN_DOES_NOT_EXIST)]
